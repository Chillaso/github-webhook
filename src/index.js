...
app.post('/checker/deploy', async (req, res) => {
    ...
    let status = await requestDeployment(req.body);
    res.sendStatus(status);

});

async function requestDeployment(body) {
    ...
    .then(response => console.log(`${response.status} - Deployment ${deployName} created for product: ${url} with id: ${id} and max price: ${price}`))
